...
const ACCESSORIES = [
  ...
  { name: "Havalandırma", img: "/havalandırma.png", slug: "havalandırma" },
    ...
    <div className="bg-[var(--secondary)] min-h-screen pb-20">
      <div className="bg-white border-b border-[var(--border)] py-2.5 px-6">
        <div className="max-w-7xl mx-auto flex items-center gap-2 text-xs font-sans text-gray-500">
          <Link to="/" className="hover:text-[var(--gaf-red)]">
            {lang === "tr" ? "Ana Sayfa" : "Home"}
          </Link>
          <ChevronRight className="w-3 h-3" />
          <span className="text-[var(--gaf-near-black)]">
            {lang === "tr" ? "Çatı Detay Aksesuarları" : "Roof Accessories"}
          </span>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {ACCESSORIES.map((item) => (
            <motion.div 
              key={item.slug} 
              whileHover={{ y: -10 }} 
              className="bg-white p-4 border hover:shadow-xl transition-shadow"
            >
              <Link to={`/products/accessories/${item.slug}`}>
                <div className="aspect-video overflow-hidden mb-4 bg-gray-100 flex items-center justify-center">
                  <img 
                    src={item.img} 
                    alt={item.name} 
                    loading="lazy"
                ...
                <span className="text-[var(--gaf-red)] text-xs font-semibold uppercase">
